(function ( exports ) {
	...
	function SpriteHandler( originalPixel ){
			this.orgPixel = originalPixel || new Pixel( 255, 255, 255, 255 );
			this.margin = 0;
		}
	SpriteHandler.prototype = {
			cut: function( imageHandler, x, y ){
						...
						if( !start.isValid() || start.isSimilar( self.orgPixel, 10 ) )
							return;
						// shit!
						for( var i = margin + 1; i--; ){
							...
							do{
								...
								imageHandler.setRect(lu, rl, function(pixel, x, y, line){
									if( !pixel.isSimilar( self.orgPixel, 10 ) && pixel.isValid() ){
										flag = true;
										switch(line) {
